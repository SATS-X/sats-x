...
import { HiOutlineUsers, HiOutlineBookOpen, HiOutlineClipboardList, HiOutlineRefresh, HiOutlineArrowLeft, HiOutlineSearch, HiOutlineMail, HiOutlinePhone, HiOutlineCalendar, HiOutlineClock, HiOutlineLocationMarker, HiOutlinePencil, HiOutlineCheck, HiOutlineX } from 'react-icons/hi'
...
const SubjectDetail = ({ subject, onBack }) => {
    ...
    const fetchStudentsForSubject = useCallback(async (subjectId) => {
    ...
    const handleSaveEdit = async (index) => {
        ...
        try {
            ...
            if (!editedTimes.start_time || !editedTimes.end_time || 
                !editedTimes.checkin_opens_at || !editedTimes.ontime_ends_at || 
            ...
            const ontime_window_minutes = ontimeEndsMinutes - checkinOpensMinutes
            const late_window_minutes = checkinClosesMinutes - checkinOpensMinutes
            ...
            // Update local state
            subject.schedules[index].start_time = editedTimes.start_time
            subject.schedules[index].end_time = editedTimes.end_time
            subject.schedules[index].time_slot = `${editedTimes.start_time} - ${editedTimes.end_time}`
            subject.schedules[index].attendance_windows = {
                ...subject.schedules[index].attendance_windows,
                early_checkin_minutes: early_checkin_minutes,
                ontime_window_minutes: ontime_window_minutes,
                late_window_minutes: late_window_minutes,
                checkin_opens_at: editedTimes.checkin_opens_at,
                ontime_ends_at: editedTimes.ontime_ends_at,
                checkin_closes_at: editedTimes.checkin_closes_at
            }
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-slate-600">{t('subject')} ID: {subject.subject_id}</p>
                            </div>
                        </div>
                    </div>
                    <button
                    ...
                    <div className="p-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
                            {subject.schedules.map((schedule, index) => (
                                <div key={index} className="group bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
                                    {/* Card Header */}
                                            ...
                                            <div className="flex items-center space-x-3">
                                                <div className="relative">
                                                    ...
                                                    <div className="relative bg-gradient-to-br from-indigo-500 to-indigo-700 text-white rounded-xl w-14 h-14 flex flex-col items-center justify-center shadow-lg">
                                                        <span className="text-2xl font-semibold leading-none">{schedule.day}</span>
                                                        <span className="text-[10px] font-normal opacity-90 uppercase">{schedule.month}/{schedule.year}</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div className="text-base font-medium text-slate-900">{schedule.day_name}</div>
                                    ...
                                    <div className="p-5 space-y-3">
                                        {editingScheduleIndex === index ? (
                                            <div className="bg-gradient-to-r from-indigo-50 to-indigo-100/50 rounded-lg p-4 border border-indigo-200">
                                                <div className="flex items-center space-x-2 mb-3">
                                                    ...
                                                    <h4 className="text-sm font-medium text-indigo-900">{t('timeSlot')}</h4>
                                                </div>
                                            ...
                                            <div className="flex items-start space-x-3">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="text-sm font-medium text-slate-900">{schedule.time_slot}</div>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-start space-x-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="text-sm font-medium text-slate-900">{schedule.room}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {schedule.attendance_windows && (
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <h3 className="text-sm font-medium text-slate-700">{t('attendanceWindow')}</h3>
                                            </div>
                                            {editingScheduleIndex === index ? (
                                                ...
                                                <div className="relative group">
                                                    ...
                                                    <div className="relative bg-white rounded-lg border border-emerald-200 p-3">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => handleTimeChange('checkin_opens_at', e.target.value)}
                                                ...
                                                <div className="relative group">
                                                    ...
                                                    <div className="relative bg-white rounded-lg border border-blue-200 p-3">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => handleTimeChange('ontime_ends_at', e.target.value)}
                                                ...
                                                <div className="relative group">
                                                    ...
                                                    <div className="relative bg-white rounded-lg border border-rose-200 p-3">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => handleTimeChange('checkin_closes_at', e.target.value)}
                                                ...
                                                <div className="group bg-gradient-to-r from-emerald-50 to-emerald-100/50 rounded-lg px-3 py-2.5 border border-emerald-200/50 hover:border-emerald-300 transition-all">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <span className="text-xs font-medium text-emerald-700">{t('checkInOpens')}</span>
                                                        </div>
                                                        <span className="text-sm font-medium text-emerald-900 font-mono">{schedule.attendance_windows.checkin_opens_at}</span>
                                                    </div>
                                                </div>
                                                <div className="group bg-gradient-to-r from-blue-50 to-blue-100/50 rounded-lg px-3 py-2.5 border border-blue-200/50 hover:border-blue-300 transition-all">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <span className="text-xs font-medium text-blue-700">{t('onTimeEnds')}</span>
                                                        </div>
                                                        <span className="text-sm font-medium text-blue-900 font-mono">{schedule.attendance_windows.ontime_ends_at}</span>
                                                    </div>
                                                </div>
                                                <div className="group bg-gradient-to-r from-rose-50 to-rose-100/50 rounded-lg px-3 py-2.5 border border-rose-200/50 hover:border-rose-300 transition-all">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <span className="text-xs font-medium text-rose-700">{t('checkInCloses')}</span>
                                                        </div>
                                                        <span className="text-sm font-medium text-rose-900 font-mono">{schedule.attendance_windows.checkin_closes_at}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
