...
- Running Backend API (`attendance-system-be`) on port `4000`

### Step-by-Step Setup

1. **Install Dependencies**:
   ```bash
   cd attendance-system-fe
   npm install
   ```
...
flowchart LR
    ...
    subgraph EC2["AWS EC2 Host"]
        NGINX -->|/* (Static Assets)| DIST["/attendance-system-fe/dist (React SPA)"]
        NGINX -->|/api/* (Reverse Proxy)| BE_DOCKER["attendance-system-be Container (:4000)"]
    end
```
...
2. **Copy Artifacts to EC2**:
   ```bash
   scp -i backend-key.pem -r dist deploy ubuntu@<BACKEND_PUBLIC_IP>:~/attendance-system-fe/
   ```

3. **Reload Production Nginx Web Container**:
   ```bash
   ssh -i backend-key.pem ubuntu@<BACKEND_PUBLIC_IP> \
     "cd ~/attendance-system-be && docker compose -f docker-compose.prod.yml up -d --build web"
   ```
...
| [**`attendance-system-be`**](../attendance-system-be) | Express, PostgreSQL, Prisma | Core REST API, Auth, Database ORM, IoT Webhooks |
| [**`attendance-system-infra`**](../attendance-system-infra) | Terraform, AWS Lambda, S3, IoT Core | Serverless AWS Cloud Infrastructure & Pipelines |
| [**`attendance-system-iot`**](../attendance-system-iot) | PlatformIO, ESP32, ESP32-CAM | Edge Hardware: Face Capture, LCD UI, Distance Trigger |
| [**`attendance-system-liveness-detection`**](../attendance-system-liveness-detection) | FastAPI, TensorFlow, WebSockets | Real-time Deep Learning Anti-Spoofing Microservice |
